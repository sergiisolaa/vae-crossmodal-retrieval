...
class Model(nn.Module):
    ...
    def trainstep(self, img, att):
        ...
        att_from_att = self.decoder[self.auxiliary_data_source](mu_att)
        ...
        ##############################################
        img_from_att = self.decoder['resnet_features'](mu_att)
        att_from_img = self.decoder[self.auxiliary_data_source](mu_img)
        ...
        if cross_reconstruction_loss>0:
           loss = loss + cross_reconstruction_factor*cross_reconstruction_loss
        ...
        return loss.item(), reconstruction_loss, cross_reconstruction_factor*cross_reconstruction_loss, distance_factor*distance
    ...
    def train_vae(self):
        ...
        for epoch in range(0, self.nepoch ):
            ...
            #for iters in range(0, 1000, self.batch_size):
                ...
                loss, lossR, lossC, lossD = self.trainstep(data_from_modalities[0], data_from_modalities[1] )
    ...
    def generate_gallery(self):
        ...
        #for iters in range(0, 500, 50):
            ...
            mu_img = self.encoder['resnet_features'](data_from_modalities[0])
            
            mu_att = self.encoder['attributes'](data_from_modalities[1])
